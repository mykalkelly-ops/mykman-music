...
- `/apple-music` can create/reuse a private "MYKMAN Comparisons" Apple Music playlist from the canonical comparison queue. This uses MusicKit browser authorization and only includes songs with `apple_library_id`.
...
- Artist pages can mark Apple catalog status manually. Use `Mark no Apple match` for feature-only/no-catalog cases so `/artists` batch enrichment skips them. Use manual Apple artist ID override when Apple has the correct artist but search-by-name fails.
...
- `/albums` uses cached album scores and renders a top slice by default, with `?all=1` and `?refresh=1` escape hatches. Avoid reintroducing uncached full album scoring plus full-table rendering on normal page loads.
